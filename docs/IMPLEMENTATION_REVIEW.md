...
| 2. Auth | `/login` — email OTP or magic link; `?next=` preserved | ✅ |
...
- **POST `/api/send-email`** — Sends eligibility summary via Resend. Not called from UI yet.
...
1. **Supabase**
   ...
   - [ ] Auth → URL Configuration: Site URL = `http://localhost:3000`, Redirect URLs include `http://localhost:3000/auth/callback`.
...
- Wire "Email me my summary" (or similar) to POST `/api/send-email`.
